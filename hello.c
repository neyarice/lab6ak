...
MODULE_AUTHOR("Pankin Vladislav");
MODULE_DESCRIPTION("Laboratory work 6");
MODULE_LICENSE("Dual BSD/GPL");

static unsigned int greet_number = 1;

module_param(greet_number, uint, S_IRUGO);

MODULE_PARM_DESC(greet_number, "Number of times 'Hello, World!' string will be printed");
...
{
    if (greet_number == 0 || (greet_number >= 5 && greet_number <= 10)) {
        printk(KERN_WARNING "Parameter = 0 or 5 <= Parameter <= 10");  
    }
